...
class RowHammer:
    ...
    def bitflip_list(val, exp):
        ...
        return [i for i, c in enumerate(expr[::-1]) if c == '1']
    ...
    def display_errors(self, row_errors, read_count, do_error_summary=False):
        ...
        err_dict = {}
        ###################################################
        # Added code to create list of banks
        ###################################################
        banks = {}
        for index in range((2 ** self.settings.geom.bankbits)):
            banks[index] = {}
        ###################################################
        for row in row_errors:
            ...
            if self.verbose or do_error_summary:
                for i, word, expected in row_errors[row]:
                    ...
                    if not (_row in banks[bank]):
                        banks[bank][_row] = {}
                    if self.verbose:
                        print(
                            "Error: 0x{:08x}: 0x{:08x} (row={}, bank={}, col={})".format(
                                addr, word, _row, bank, col))
                    ###################################################
                    # Add bank, row, col
                    bitflips = self.bitflip_list(word, expected)
                    # cols[col] = bitflips
                    banks[bank][_row][col] = bitflips
